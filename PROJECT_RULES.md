...
### Entry - 2026-05-11 (Commit-aware UPDATEUI status + git-safe repo updates)
- Date: 2026-05-11
...
### Entry - 2026-05-19 (PowerShell UI width must use effective pane width)
...
### Entry - 2026-07-15 (Full Cleanup must not overlap or hide Windows Update servicing)

- Date: 2026-07-15
- Problem: A clean post-update Full Cleanup displayed the `/ResetBase` header for more than 40 minutes while an earlier `RestoreHealth` Windows Update/FOD acquisition remained stuck at 49%; manual cancellation failed, the servicing backend survived the visible runner, and the subsequent restart required a power cut.
- Root cause: Full Cleanup ran `RestoreHealth` with online source access, immediately started `/ResetBase` after the DISM client reported success even though a CBS/WULib acquisition remained active, and ignored required-step failures instead of aborting the later cleanup stages.
- Guardrail/rule: Full Cleanup must run `RestoreHealth /LimitAccess`, block when reboot/pending-servicing flags or active `DISM`/`DismHost`/`TiWorker` processes exist, and abort the entire sequence after any required-step failure. Never auto-force-kill Windows servicing from the normal app flow; report `FULL CLEANUP STOPPED SAFELY` and leave later stages untouched.
- Files affected: `FullCleanup.cmd`, `SystemCleanup.cmd`, `SystemCleanup.ps1`, `ManageUpdates.ps1`, `app-metadata.json`, `README.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
- Validation/tests run: PowerShell parser validation passed for every `.ps1`; synthetic batch failure returned native exit code `2`, logged the failure/safe stop, and proved no `DISM` or `CleanInFlight` stage started; synthetic `TiWorker` preflight returned code `20` before any step; the PowerShell preflight returned exactly one string blocker for synthetic `TiWorker`; JSON, line endings, `git diff --check`, and static diff review passed; local-source installed update completed with exit code `0`; installed runtime SHA-256 hashes matched the repo, installed parser validation passed, metadata readback showed `1.0.4`, and both context-menu registry commands remained correct.

### Entry - 2026-07-15 (RestoreHealth fallback requires an exact verified repair source)

- Date: 2026-07-15
- Problem: `/RestoreHealth /LimitAccess` prevents hidden Windows Update acquisition, but a damaged or incomplete local component store still needs a safe, actionable repair path.
- Root cause: The app had no guarded way to validate an ISO against the live installation or let the user explicitly opt into Windows Update after local-only repair failed.
- Guardrail/rule: After local-only RestoreHealth fails, offer verified ISO repair as the default, explicit `W` for Windows Update, and `ESC` for safe stop. Never trust an ISO filename: require one exact `install.wim` / `install.esd` match for edition, installation type, architecture, build/revision, and base language. Choose a free mount letter from `Z:` downward, never replace an occupied drive, remember verified paths only in preserved ignored state, and dismount only media mounted by the app. After any fallback repair succeeds, stop before `/ResetBase` and require a normal restart plus a fresh Full Cleanup run.
- Files affected: `ManageUpdates.ps1`, `FullCleanup.cmd`, `SystemCleanup.cmd`, `SystemCleanup.ps1`, `app-metadata.json`, `README.md`, `CHANGELOG.md`, `PROJECT_RULES.md`.
- Validation/tests run: Elevated live comparison confirmed `Professional / Client / x64 / 26200.8875 / en-US` for both Windows and WIM index 1; detached-media smoke mounted at `Z:`, validated, persisted state, and dismounted; a synthetic occupied `Z:` selected `Y:`; an invalid non-ISO source was blocked with exit `32`; synthetic `pending.xml` batch preflight returned exit `20` before SFC/DISM; all PowerShell parsers, JSON/version consistency, batch labels, CRLF/LF policy, and `git diff --check` passed; local-source installed update returned `0`, installed hashes/parsers/state matched, and both context-menu registry commands remained correct.
